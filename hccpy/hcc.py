...
class HCCEngine:
    ...
    def __init__(
        ...
        fnmaps = {
            ...
            "23": {
                "dx2cc": {
                    "2018": "data/F2318P1Q.TXT",
                    "2019": "data/F2319P1Q.TXT",
                    "Combined": "data/F23_AllYearsCombined.TXT",
                },  # CMS has not provided updated mappings
            ...
            "ESRDv21": {
                "dx2cc": {"2019": "data/F2118H1R.txt", "Combined": "data/F2118H1R.txt"},
        ...
        assert fnmaps[version]["dx2cc"].get(
            dx2cc_year
        ), "Invalid combination of version and year parameters"
    ...
    def _df_profile(self, df, cc_direct=False, num_chunks=2, nb_workers=None):
        ...
        for i in range(0, len(df), chunk_size):
            ...
            chunk[["disabled", "origds", "elig"]] = (
                ...
                .parallel_apply(
                    lambda x: AGESEXV2.get_ds(
                        x["age"], x["orec"], x["medicaid"], x["elig"]
                    ),
                    axis=1,
                )
                .tolist()
            ...
            if "ESRD" not in self.version:
                chunk["hcc_age_lst"] = chunk[
                    ...
                    lambda x: V2218O1P.get_risk_dct(
                        self.coefn,
                        x["hcc_lst"],
                        x["age"],
                        x["sex"],
                ...
                chunk["hcc_age_lst"] = chunk[
                    ...
                    lambda x: E2118P1P.get_risk_dct(
                        self.coefn,
                        x["hcc_lst"],
                        x["age"],
                        x["sex"],
    ...
    def _list_profile(self, dx_lst, age, sex, elig, orec, medicaid, cc_direct=False):
        ...
        if "ESRD" not in self.version:
            risk_dct = V2218O1P.get_risk_dct(
                self.coefn, hcc_lst, age, sex, elig, origds, medicaid
            )
        else:
        ...
        score_age = round(
            np.sum([v for k, v in risk_dct.items() if re.search(demo_pttrn, k)]), 4
        )
